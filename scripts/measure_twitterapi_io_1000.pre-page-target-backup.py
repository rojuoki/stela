...
through 7 days → 30 days → 120 days → the end of that calendar year; a dense
window keeps its current width. Split children never change that normal-window
schedule. Observed exhaustion is an operational coverage signal, not a claim
of provider-level completeness.
"""
...
COLLECT_STEP_30_DAYS = 30
COLLECT_STEP_120_DAYS = 120
...
class TwitterApiIoClient:
    ...
    def _request_json(
        ...
        for attempt in range(1, self.args.retries + 2):
            ...
            if self.estimated_cost_usd() >= self.args.max_estimated_cost_usd:
                raise PrototypeFailure(
...
class OldestBlockExperiment:
    def __init__(
        ...
        self.collect_span_days = args.window_days
        self.adaptive_step_to_30_count = 0
        self.adaptive_step_to_120_count = 0
        self.adaptive_step_to_year_end_count = 0
        self.adaptive_keep_dense_count = 0
    ...
    def restore_checkpoint(self) -> bool:
        ...
        self.adaptive_step_to_30_count = self._int_or_zero(
            counters.get("adaptive_step_to_30_count")
        )
        self.adaptive_step_to_120_count = self._int_or_zero(
            counters.get("adaptive_step_to_120_count")
        )
        self.adaptive_step_to_year_end_count = self._int_or_zero(
            counters.get("adaptive_step_to_year_end_count")
        )
        self.adaptive_keep_dense_count = self._int_or_zero(
            counters.get("adaptive_keep_dense_count")
        )
        ...
        if saved_span_days >= 1:
            self.collect_span_days = saved_span_days
    ...
    def save_checkpoint(self) -> None:
        ...
        payload = {
            ...
            "counters": {
                ...
                "adaptive_step_to_30_count": self.adaptive_step_to_30_count,
                "adaptive_step_to_120_count": self.adaptive_step_to_120_count,
                "adaptive_step_to_year_end_count": (
                    self.adaptive_step_to_year_end_count
                ),
                "adaptive_keep_dense_count": self.adaptive_keep_dense_count,
            ...
            "resume": {
                ...
                "collect_span_days": self.collect_span_days,
                "next_collect_start": (
    ...
    def save_progress_snapshot(self) -> None:
        ...
        payload = {
            ...
            "config": {"target_count": self.args.target_count},
            "result": {
                "status": "IN_PROGRESS",
    ...
    def probe_until_collect(self) -> int | None:
        ...
        created = datetime.fromtimestamp(self.lower_bound, UTC)
    ...
    def _next_collect_span_days(
        self,
        window: Window,
        unique_new: int,
    ) -> tuple[int, str]:
        """Mirror the official bulk collector's normal-window schedule.

        The decision is based only on the fully resolved root window. A capped
        root is discarded before its children are fetched, so child recursion
        cannot make an untrusted parent look sparse or dense.
        """
        current = self.collect_span_days
        dense = unique_new >= COLLECT_DENSE_POST_THRESHOLD
        if dense:
            self.adaptive_keep_dense_count += 1
            return current, "keep_dense"
        if current == self.args.window_days:
            self.adaptive_step_to_30_count += 1
            return COLLECT_STEP_30_DAYS, "step_to_30"
        if current == COLLECT_STEP_30_DAYS:
            self.adaptive_step_to_120_count += 1
            return COLLECT_STEP_120_DAYS, "step_to_120"
        if current == COLLECT_STEP_120_DAYS:
            # This span begins after the just-resolved 120-day window, so
            # calculate from that *next* cursor position and land on Jan 1.
            next_start = window.end
            next_dt = datetime.fromtimestamp(next_start, UTC)
            next_year = datetime(next_dt.year + 1, 1, 1, tzinfo=UTC)
            remaining_seconds = max(0, epoch(next_year) - next_start)
            self.adaptive_step_to_year_end_count += 1
            return (
                max(
                    self.args.window_days,
                    math.ceil(remaining_seconds / (24 * 60 * 60)),
                ),
                "step_to_end_of_year",
            )
        self.adaptive_reset_to_7_count += 1
        return self.args.window_days, "reset_to_7"
    ...
    def collect_from(self, collect_start: int) -> None:
        ...
        while cursor < self.snapshot_end and not self.target_reached():
                ...
                self.snapshot_end,
                cursor + self.collect_span_days * 24 * 60 * 60,
            )
            ...
            if self.last_collect_used_split:
                ...
                # its observed page density. Resume the normal route at 7d
                # after the split range is fully resolved.
                self.adaptive_reset_to_7_count += 1
                next_span_days, action = self.args.window_days, "reset_after_split"
                self.last_collect_used_split = False
            else:
                next_span_days, action = self._next_collect_span_days(window, unique_new)
            self.client._log(
                "ADAPTIVE "
                f"span_days={self.collect_span_days} unique_new={unique_new} "
                f"action={action} next_span_days={next_span_days}"
            )
            self.collect_span_days = next_span_days
            cursor = window.end
    ...
    def _resolve_split_range(
        ...
        while pending:
            ...
            if result.status == "RESOLVED":
                ...
                self.collect_span_days = min(
                    SPLIT_RECOVERY_MAX_DAYS,
                    max(1, math.ceil(next_width / (24 * 60 * 60))),
                )
            ...
            # partial child, then the range that followed it.
            pending[0:0] = [older]
            if newer[0] < newer[1]:
                pending.insert(1, newer)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.0,
        help="stop before the estimated TwitterAPI.io spend reaches this amount (default: 2.00)",
    )
    parser.add_argument(
        "--checkpoint",
        type=Path,
...
def run(args: argparse.Namespace) -> int:
    ...
    lower_bound = epoch(parse_datetime(profile_created))
    print(
        f"TwitterAPI.io; @{username}; created={iso_epoch(lower_bound)}; "
        f"probe_limit={args.probe_limit}; collect_spans="
        f"{args.window_days}/{COLLECT_STEP_30_DAYS}/{COLLECT_STEP_120_DAYS}/year-end; "
    ...
    payload = {
        ...
        "config": {
            ...
            "collect_step_days": [
                args.window_days,
                COLLECT_STEP_30_DAYS,
                COLLECT_STEP_120_DAYS,
            ],
        ...
        "metrics": {
            ...
            "collect_span_days_at_stop": experiment.collect_span_days,
            "adaptive_step_to_30_count": experiment.adaptive_step_to_30_count,
            "adaptive_step_to_120_count": experiment.adaptive_step_to_120_count,
            "adaptive_step_to_year_end_count": (
                experiment.adaptive_step_to_year_end_count
            ),
            "adaptive_keep_dense_count": experiment.adaptive_keep_dense_count,
