...
export function getTweetsByAccount(accountId: string): Tweet[] {
  ...
  // Parse media_json for each tweet
  return tweets.map(tweet => ({
    ...tweet,
    media: tweet.media_json ? parseMediaJson(tweet.media_json) : undefined,
  }));
}
 ...
 * Based on actual usage: 9 calls = $1.5 → $0.167 per call
 */
export const COST_PER_CALL_USD = 0.167; // $0.167 / real API call (actual measured cost)
