...
export default function Home() {
  ...
  const handleUnlock = async (force = false) => {
    ...
    // 既にアカウントデータがある場合は、状態を保持する（再フェッチしない）
    if (!accountData) {
      console.log(`[excavate] アカウントデータなし - ユーザー情報取得: ${raw}`);
      await manualLookup(raw);
    } else {
      console.log(`[excavate] アカウントデータ保持 - 再フェッチなし: ${accountData.username}`);
    }
  ...
  return (
    <main className="max-w-2xl mx-auto px-4 py-12">
      ...
      {/* 1. SearchBar (input + Lookup/Excavate buttons) */}
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              // 入力変更時はアカウント状態をリセットしない（必要時のみ）
              // draft stateは inputだけに閉じる
            }}
      ...
      {/* 2. AccountCard (固定表示 - Lookup/Excavate中も保持) */}
      ...
      {/* 3. ResultArea - Job Status, Chart, Tweets */}
      <JobStatus
        status={status}
        jobPhase={jobPhase}
        jobInfo={jobInfo}
